...
class Min_mh extends Task {

	function mh_Saobshteniq() {
		echo "> Проверявам за съобщения в МЗ\n";
		$this->setSession(21,0);

		$html = $this->loadURL("http://www.mh.government.bg/AllMessages.aspx",0);
		if (!$html) return;
	...
	function mh_Novini() {
		...
		$this->setSession(21,1);

		$html = $this->loadURL("http://www.mh.government.bg/News.aspx?pageid=401",1);
		if (!$html) return;
	...
	function mh_Normativni() {
		...
		$this->setSession(21,2);

		$html = $this->loadURL("http://www.mh.government.bg/Articles.aspx?lang=bg-BG&pageid=393",2);
		if (!$html) return;
	...
	function mh_Naredbi() {
		...
		$this->setSession(21,3);

		$html = $this->loadURL("http://www.mh.government.bg/Articles.aspx?lang=bg-BG&pageid=391",3);
		if (!$html) return;
	...
	function mh_Postanovleniq() {
		...
		$this->setSession(21,4);

		$html = $this->loadURL("http://www.mh.government.bg/Articles.aspx?lang=bg-BG&pageid=381",4);
		if (!$html) return;
		$items = $this->xpathDoc($html,"//table[@id='ctl00_ContentPlaceClient_ucArticlesList_gvArticles']//tr[not(@class)]/td/a[@class='list_article_title']");
	...
	function mh_Otcheti() {
		...
		$this->setSession(21,5);

		$html = $this->loadURL("http://www.mh.government.bg/Articles.aspx?lang=bg-BG&pageid=532",5);
		if (!$html) return;
		$items = $this->xpathDoc($html,"//table[@id='ctl00_ContentPlaceClient_ucArticlesList_gvArticles']//tr[not(@class)]/td/a[@class='list_article_title']");
