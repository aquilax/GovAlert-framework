...
class Images {
	...
	static function resizeItemImage($filename, $type)
	{
		...
		$source = self::loadResizeItemImage($filename);
		if (!$source)
			return;
		...
		imagedestroy($source);
		self::saveResizeItemImage($thumb, $filename, $type);
	}

	static function fitinItemImage($filename, $type, $options)
	{
		ini_set('memory_limit', '150M');
	...
	static function loadResizeItemImage($filename)
	{
