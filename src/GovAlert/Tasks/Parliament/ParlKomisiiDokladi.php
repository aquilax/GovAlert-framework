...
class ParlKomisiiDokladi extends Base
{
	...
	function execute($html)
	{
		...
		while ($row = $res->fetch_array()) {
			...
			$checks[] = array("http://parliament.bg/bg/parliamentarycommittees/members/" . $row[0] . "/reports/period/" . date("Y-m", $this->timeDiff('-1 month')), $row[1]);
		}
		$res->free();
